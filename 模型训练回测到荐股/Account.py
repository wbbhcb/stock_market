...
class Account:
    ...
    def sell_trigger(self, stock_id, day, all_df, index_df):
        ...
        # print(day, stock_id)
        # 可能会有一些停牌企业，后期再改
        idx_stcok = (all_df['trade_date'] == day) & (all_df['ts_code'] == stock_id)
        # print(all_df[idx])
        if len(all_df[idx_stcok]) == 0: # 停牌
            return False, 3, 0
        low = all_df[idx_stcok]['low'].values[0]
        high = all_df[idx_stcok]['high'].values[0]
        open = all_df[idx_stcok]['open'].values[0]
        close = all_df[idx_stcok]['close'].values[0]

        idx = self.stock_id.index(stock_id)
        
        tmp_rate = (open - self.stock_price[idx]) / self.stock_price[idx]
        
        # stock_sell_flag = 1就开盘卖出， =2 就收盘卖出
        stock_sell_flag = all_df[idx_stcok]['sell_flag'].values[0]
        # stock_sell_flag = open < all_df[idx_stcok]['ma13']
        # print(stock_sell_flag.values)
        if tmp_rate <= self.stop_loss_rate or index_df['sell_flag']==1 or stock_sell_flag == 1:  # 止损卖出，开盘价卖出
            if high == low: # 说明是跌停价, 跌停价则无法卖出
                print(open, high, low, close)
                return False, 3, 0
            return True, 2, open
        elif tmp_rate >= self.stop_profit_rate or index_df['sell_flag']==1 or stock_sell_flag == 1:  # 止盈卖出，开盘价卖出
            return True, 1, open
        ...
        # stock_sell_flag = close < all_df[idx_stcok]['ma13']
        # 收盘指数大跌的话卖出
        if index_df['sell_flag']==2 or stock_sell_flag == 2:
            tmp_rate = (close - self.stock_price[idx]) / self.stock_price[idx]
            if high == low: # 说明是跌停价, 跌停价则无法卖出
                print(open, high, low, close)
                return False, 3, 0
            if tmp_rate < 0:
    ...
    def BackTest(self, buy_df, all_df, index_df, buy_price='close'):
        ...
        :return:
        """
        day_info = index_df['trade_date_id']
        day_time = index_df['trade_date']
        for i in range(len(day_info)):
            day = day_info[i]
            tmp_idx = buy_df['trade_date_id'] == day
            ...
            if len(tmp_df) != 0:
                for j in range(len(tmp_df)):
                    # if tmp_df['buy_flag'][j] == 0: # 如果buy_flag=0，表示不满足均线买入法则
                    #     continue
                    # if len(self.stock_name)>=7:
                    #     break
                    # if tmp_df['buy_flag'][j] == 0:
                    #     money = self.market_value * 0.1
                    # else:
                    ...
                    if buy_num == 0:
                        continue
                    buy_num = buy_num * 100
                    self.buy_stock(day_time[i], tmp_df['name'][j],
            ...
            for j in range(len(self.stock_id) - 1, -1, -1):
                # print(self.buy_date[j], day_time[i])
                if self.buy_date[j] == day_time[i]:
                    continue
                stock_id = self.stock_id[j]
                stock_name = self.stock_name[j]
                sell_num = self.stock_num[j]  # 假设全卖出去

                is_sell, sell_kind, sell_price = self.sell_trigger(stock_id, day_time[i], 
                                                                   all_df, index_df.loc[i])
                if is_sell:
                    self.sell_stock(day_time[i], stock_name, stock_id, sell_price, sell_num, sell_kind)

            # 更新持股周期及信息
            self.update(day_time[i], all_df)
